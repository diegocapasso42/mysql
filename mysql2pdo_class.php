 ...
 */
 ...
 class mysql2pdo
 {
    ...
    /**
     * Map PDO::TYPE_* to mysql2pdo Type
     *
    ...
    protected function _getLastLink($link, $validate = true)
    {
        ...
        if ($validate === true && !isset($this->_instances[$link]) || empty($this->_instances[$link])) {
            if (isset($this->_instances[$link])) {
                throw new mysql2pdo_exception($this->_params[$link]['error'], $this->_params[$link]['errno']);
            } else {
                throw new mysql2pdo_exception('No db at  instance #' . ($link - 1));
            }
        }
        return $link;
    }
